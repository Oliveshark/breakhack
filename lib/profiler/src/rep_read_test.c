...
static const char *print_allocation_type(AllocationType type)
{
    const char *result;
    switch (type) {
        case AllocType_none:
...
static void handle_allocation(ReadParams *params, uint8_t **buffer)
{
...
static void handle_deallocation(ReadParams *params, uint8_t *buffer)
{
    switch (params->alloc_type) {
        case AllocType_malloc:
            free(buffer);
...
static void test_fread(RepetitionTester *tester, ReadParams *params)
{
...
static void test_read(RepetitionTester *tester, ReadParams *params)
{
    while (rept_is_testing(tester)) {

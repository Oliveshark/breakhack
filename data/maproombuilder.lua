...
local function build_coridoor_room(room)
	...
	for _, exit in ipairs(room.exits) do
		if UP == exit then
			exits.up = true
		elseif DOWN == exit then
			exits.down = true
		elseif LEFT == exit then
			exits.left = true
		elseif RIGHT == exit then
			exits.right = true
		end
	end
	...
	-- Build the coridoors
	if exits.down then
		build_vert_center_coridoor(room, 7)
	end
	if exits.up then
		build_vert_center_coridoor(room, 0)
	end
	if exits.left then
		build_horiz_center_coridoor(room, 0)
	end
	if exits.right then
		build_horiz_center_coridoor(room, 9)
	end
...
local function build_normal_room(room)
	...
	elseif (CURRENT_LEVEL > 3 or QUICK_MODE) and random(8) == 1 then
		...
		room.modifier.arg = ""
	elseif
		((not pitsAdded and not crumbling and (CURRENT_LEVEL > 1 or QUICK_MODE)) or CURRENT_LEVEL > 3)
		and random(8) == 1
	then
...
function module.load_textures(map)
	local t_floor = add_texture(map, "Objects/Floor.png")

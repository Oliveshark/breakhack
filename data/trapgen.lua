...
local traps = {}
for i = 1, 7 do
	local trap = { textures[1], textures[2], i * 16, 3 * 16, CURRENT_LEVEL * 4 }
...
function module.add_traps_to_room(room)
	...
	while i < count do
		...
		if CURRENT_LEVEL < 4 then
			if
				room_builder.is_tile_avilable(room, rx, ry)
...
function module.load_traps(map, room_traps)
	for i = 0, 15 do
		for j = 0, 11 do
			local trap = room_traps[i][j]
			if trap then

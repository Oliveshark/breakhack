...
void
map_room_modifier_player_effect(Player *player, RoomMatrix *matrix, Vector2d *direction,
                                void (*move_cb)(Player *, RoomMatrix *, Vector2d))
{
	Position matrixPos = position_to_matrix_coords(&player->sprite->pos);
	if (matrix->modifier->type == RMOD_TYPE_WINDY && player->effects.effect != POTION_STONE
	    && !vector2d_equals(*direction, VECTOR2D_NODIR) && matrixPos.x > 0 && matrixPos.x < MAP_ROOM_WIDTH - 1

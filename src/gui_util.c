...
render_frame_on_texture(Uint32 width, Uint32 height, Position offset, Camera *cam)
{
	...
	texture->dim = (Dimension){width * 16, height * 16};
	...
	for (i = 0; i < width; ++i) {
		for (j = 0; j < height; ++j) {
			...
			if (i == 0 && j == 0) {
				texture_render_clip(source, &box, &frame_top_left, cam);
			} else if (i == (width - 1) && j == 0) {
				texture_render_clip(source, &box, &frame_top_right, cam);
			} else if (i == 0 && j == (height - 1)) {
				texture_render_clip(source, &box, &frame_bottom_left, cam);
			} else if (i == (width - 1) && j == (height - 1)) {
				texture_render_clip(source, &box, &frame_bottom_right, cam);
			} else if (i == 0) {
				texture_render_clip(source, &box, &frame_left, cam);
			} else if (i == (width - 1)) {
				texture_render_clip(source, &box, &frame_right, cam);
			} else if (j == 0) {
				texture_render_clip(source, &box, &frame_top, cam);
			} else if (j == (height - 1)) {
				texture_render_clip(source, &box, &frame_bottom, cam);
			} else {
				texture_render_clip(source, &box, &frame_center, cam);
			}
		}
	}
...
Sprite *
gui_util_create_frame_sprite(Uint32 width, Uint32 height, Camera *cam)
{
	return render_frame_on_texture(width, height, POS(16, 16 * 10), cam);
}

Sprite *
gui_util_create_tooltip_frame_sprite(Uint32 width, Uint32 height, Camera *cam)
{
